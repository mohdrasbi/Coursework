...
int main(int argc, char **argv)
{
	...
	//Requester threads
	pthread_t *reqid = malloc(numReq * (sizeof(pthread_t)));
	struct reqthread args[numReq];

	//Resolver threads
	pthread_t *resid = malloc(numRes * (sizeof(pthread_t)));
	...
	if(numReq < numFiles)
	{
		...
		for(int i = 0; i < numFiles; i++)
		{			
			args[j].datafile = dataFiles[i];
			args[j].filename = argv[i + 5];
			args[j].sharedArray = arr;
			args[j].index = &indexInArr;
			args[j].arrSize = size;
			args[j].id = i;
			args[j].logfile = reqLogFile;
			args[j].mutex = &arr_mutex;
			args[j].domain = &domain;
			args[j].space = &space;
			pthread_create(&reqid[j], NULL, requesters, &args[j]);
			
			tmpNumTh ++;

			if((j == numReq - 1) || (i == numFiles - 1))
			{
				//printf("%s\n", argv[i+5]);
				j = -1;
	...
	double runtime = (end.tv_sec - begin.tv_sec) + ((end.tv_usec - begin.tv_usec)/1000000.0);
