 ...
 * 
 * Mohamed Al-Rasbi - moal7202
 * Firas Al Mahrouky - fial3560
 * 
 */
#include <stdio.h>
#include <stdlib.h>
...
*/
void eval(char *cmdline) //Exact string typed
{
	char *argv[MAXARGS]; // maybe should be maxline?
	sigset_t mask;
	sigemptyset(&mask);
	...
	if(!is_builtin_cmd(argv))
	{
		...
		{ //child process
			...
			if (execve(argv[0], argv, environ) < 0) 
			{
				...
				exit(0); // child terminates if command not found
			}
		}
		...
		{
			// job is a background job, take more input
...
int is_builtin_cmd(char **argv)
{
    ...
    if(!strcmp(arg, "exit"))
    { //strcmp returns 0 if equal, so check !strcmp
    	do_exit();
    	return 1;
    } 
    else if (!strcmp(arg, "bg") || !strcmp(arg, "fg"))
    {
    	do_bgfg(argv);
    	return 1;
    } 
...
void do_bgfg(char **argv) 
{
	...
	if (isdigit(argv[1][0])) 
	{
		// if true, the number is a PID
		pid_t p = atoi(argv[1]); // pid that needs to be resumed

		if (!(j = getprocessid(jobs, p))) 
		{
	...
	{
		// if true, the number after J is a job ID
		char* tmp = argv[1];
		memmove(tmp, tmp+1, strlen(tmp)); // takes J out of the string so that it can be converted to an int
		jid = atoi(tmp);
		
		if (!(j = getjobid(jobs, jid))) {
	...
	{
		printf("Incorrect value passed. Needs PID or JID");
		j = NULL;
	}
	...
	if (!strcmp(argv[0], "fg")) 
	{
		// process should be set to run in foreground
		j->state = FG;
		waitfg(j->pid);
	} 
	else 
	{
		// process should be set to run in background
		j->state = BG;
...
void sigchld_handler(int sig) 
{
	...
	while ((pid = waitpid(-1, &status, WNOHANG|WUNTRACED)) > 0) 
	{
		...
		{ // if receives SIGTSTP, updates state and prints information about process
			struct job_t* j = getprocessid(jobs, pid);
			j->state = ST;
			printf("Job [%d] (%d) Stopped by signal %d\n", get_jid_from_pid(pid), pid, WSTOPSIG(status));
		} 
		else if (WIFSIGNALED(status)) 
		{ // if child terminates from signal, remove from job list
			printf("Job [%d] (%d) Terminated by signal %d\n", get_jid_from_pid(pid), pid, WTERMSIG(status));
			removejob(jobs, pid);
		} 
		else if (WIFEXITED(status)) 
		{ // if child terminates on its own accord
			removejob(jobs, pid);
		}
	}
	return;
...
void sigint_handler(int sig) 
{
	// procedure: get foreground job, send SIGINT to all processes in FG job's process group (aka itself and all its children)
...
void sigtstp_handler(int sig) 
{
	...
	{
		kill(-fgID, sig); // send stop signal to foreground process and all children
	}
    return;
}
